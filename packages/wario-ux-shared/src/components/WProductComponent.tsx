...
function WProductComponent({
  ...
  sx,
  ...other
}: WProductComponentProps & BoxProps) {
  const productInstance = useMemo(
    () => catalogSelectors.productInstance(productMetadata.pi[0]),
    [catalogSelectors, productMetadata.pi],
  );
  const adornmentHTML = useMemo(
    () =>
      allowAdornment && productInstance && productInstance.displayFlags[displayContext].adornment
        ? productInstance.displayFlags[displayContext].adornment
        : '',
    [allowAdornment, productInstance, displayContext],
  );
  const descriptionHTML = useMemo(
    () => (description && productMetadata.description ? productMetadata.description : ''),
  ...
  const optionsSections = useMemo(() => {
    if (
      !description ||
      !productInstance ||
      productInstance.displayFlags[displayContext].suppress_exhaustive_modifier_list
    ) {
      return [[]];
    }
    ...
    displayContext,
    productInstance,
    productMetadata.exhaustive_modifiers,
  ...
  const priceText = useMemo(() => {
    if (productInstance && productMetadata.incomplete) {
      switch (productInstance.displayFlags[displayContext].price_display) {
        case PriceDisplay.FROM_X:
          return `from ${MoneyToDisplayString(productMetadata.price, false)}`;
        case PriceDisplay.VARIES:
          return 'MP';
        case PriceDisplay.MIN_TO_MAX: {
          ...
          return prices.length > 1 && prices[0] !== prices[prices.length - 1]
            ? `from ${MoneyToDisplayString(prices[0], false)} to ${MoneyToDisplayString(prices[prices.length - 1], false)}`
            : MoneyToDisplayString(prices[0], false);
        }
        case PriceDisplay.LIST:
          return ComputePotentialPrices(productMetadata, catalogSelectors)
            .map((x) => MoneyToDisplayString(x, false))
            .join('/');
        case PriceDisplay.ALWAYS:
        default:
  ...
  return (
    <Box component="div" {...other} sx={adornmentHTML ? [...spreadSx(sx), AdornedSxProps] : sx}>
      ...
      <Box sx={{ position: 'relative', ...(price ? { mr: `${(priceText.length * 0.6).toString()}em` } : {}) }}>
      ...
      {description &&
        optionsSections.map((option_section, l) => (
          <ProductDescription key={l}>
            <>
              {productMetadata.is_split ? (
                <span>
                  <strong>{option_section[0]}: </strong>
                </span>
              ) : (
                ''
              )}
