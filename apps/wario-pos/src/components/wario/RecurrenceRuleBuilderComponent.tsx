...
export type RecurrenceRuleBuilderComponentProps = ValSetVal<IRecurringInterval | null> &
...
const RecurrenceRuleBuilderComponent = (props: RecurrenceRuleBuilderComponentProps) => {
  ...
  const [frequency, setFreqency] = useState<Frequency>(
    props.value?.rrule ? RRule.fromString(props.value.rrule).options.freq : Frequency.WEEKLY,
  );
  const [rInterval, setRInterval] = useState<number>(
    props.value?.rrule ? RRule.fromString(props.value.rrule).options.interval : 1,
  );
  const [byWeekDay, setByWeekDay] = useState<Weekday[]>(
    props.value?.rrule ? RRule.fromString(props.value.rrule).options.byweekday.map((x) => new Weekday(x)) : [],
  );
  const [byMonth, setByMonth] = useState<number[]>(
    props.value?.rrule ? RRule.fromString(props.value.rrule).options.bymonth : [],
  );
            ...
            <>
              ...
              <Grid size={6}>
                <Autocomplete
                  ...
                  value={byWeekDay}
                  onChange={(_, v: Weekday[]) => {
                    setByWeekDay(v.sort((a, b) => a.getJsWeekday() - b.getJsWeekday()));
                  }}
              ...
              <Grid size={6}>
                <TimePicker
                  ...
                  onChange={(e: Date | number | null) => {
                    if (e !== null && isValid(e)) {
                      setLocalInterval({ ...localInterval, start: differenceInMinutes(e, startOfDay(Date.now())) });
                    }
                  }}
            ...
            <>
              ...
              <Grid size={6}>
                <DateTimePicker
                  ...
                  onChange={(date: Date | null) => {
                    setLocalInterval({
                      start: date && date.valueOf() > 0 ? setMilliseconds(setSeconds(date, 0), 0).valueOf() : -1,
                      end: localInterval.end,
                    });
                  }}
              ...
              <Grid size={6}>
                <DateTimePicker
                  ...
                  onChange={(date: Date | null) => {
                    setLocalInterval({
                      start: localInterval.start,
                      end: date && date.valueOf() > 0 ? setMilliseconds(setSeconds(date, 0), 0).valueOf() : -1,
                    });
                  }}
                  format="MMM dd, y hh:mm a"
                />
