...
const ProductCopyContainerInner = ({
  productEntry,
  onCloseCallback,
}: {
  ...
  const [orderGuideSuggestionFunctions, setOrderGuideSuggestionFunctions] = useState(
    productEntry.product.displayFlags.order_guide.suggestions,
  );
  const [orderGuideWarningFunctions, setOrderGuideWarningFunctions] = useState(
    productEntry.product.displayFlags.order_guide.warnings,
  );
  const [showNameOfBaseProduct, setShowNameOfBaseProduct] = useState(
    productEntry.product.displayFlags.show_name_of_base_product,
  );
  ...
  const [expandedPanels, setExpandedPanel] = useIndexedState(
    useState(Array<boolean>(productEntry.instances.length).fill(false)),
  );
  const [indexOfBase, setIndexOfBase] = useState(productEntry.instances.indexOf(productEntry.product.baseProductId));
  const [copyPIFlags, setCopyPIFlag] = useIndexedState(
    useState(Array<boolean>(productEntry.instances.length).fill(true)),
  );
  const [piDisplayNames, setPiDisplayName] = useIndexedState(
    useState(
      productEntry.instances.map((pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayName),
    ),
  );
  const [piDescriptions, setPiDescription] = useIndexedState(
    useState(
      productEntry.instances.map((pi) => (catalogSelectors.productInstance(pi) as IProductInstance).description),
    ),
  );
  const [piShortcodes, setPiShortcode] = useIndexedState(
    useState(productEntry.instances.map((pi) => (catalogSelectors.productInstance(pi) as IProductInstance).shortcode)),
  );
  const [piOrdinals, setPiOrdinal] = useIndexedState(
    useState(
      productEntry.instances.map((pi) => (catalogSelectors.productInstance(pi) as IProductInstance).ordinal || 0),
    ),
  );
  const [piModifierss, setPiModifiers] = useIndexedState(
    useState(productEntry.instances.map((pi) => (catalogSelectors.productInstance(pi) as IProductInstance).modifiers)),
  );
  const [piExteralIdss, setPiExternalIds] = useIndexedState(
    useState(
      productEntry.instances.map((pi) => (catalogSelectors.productInstance(pi) as IProductInstance).externalIDs),
    ),
  );
  const [piHideFromPoses, setPiHideFromPos] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.pos.hide,
      ),
    ),
  );
  const [piPosNames, setPiPosNames] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.pos.name,
      ),
    ),
  );
  const [piPosSkipCustomizations, setPiPosSkipCustomization] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.pos.skip_customization,
      ),
    ),
  );
  const [piMenuOrdinals, setPiMenuOrdinal] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.menu.ordinal,
      ),
    ),
  );
  const [piMenuHides, setPiMenuHide] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.menu.hide,
      ),
    ),
  );
  const [piMenuPriceDisplays, setPiMenuPriceDisplay] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.menu.price_display,
      ),
    ),
  );
  const [piMenuAdornments, setPiMenuAdornment] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.menu.adornment,
      ),
    ),
  );
  const [piMenuSuppressExhaustiveModifierLists, setPiMenuSuppressExhaustiveModifierList] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) =>
          (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.menu
            .suppress_exhaustive_modifier_list,
      ),
    ),
  );
  const [piMenuShowModifierOptionss, setPiMenuShowModifierOptions] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.menu.show_modifier_options,
      ),
    ),
  );
  const [piOrderOrdinals, setPiOrderOrdinal] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.order.ordinal || 0,
      ),
    ),
  );
  const [piOrderMenuHides, setPiOrderMenuHide] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.order.hide,
      ),
    ),
  );
  const [piOrderSkipCustomizations, setPiOrderSkipCustomization] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.order.skip_customization,
      ),
    ),
  );
  const [piOrderPriceDisplays, setPiOrderPriceDisplay] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.order.price_display,
      ),
    ),
  );
  const [piOrderAdornments, setPiOrderAdornment] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) => (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.order.adornment,
      ),
    ),
  );
  const [piOrderSuppressExhaustiveModifierLists, setPiOrderSuppressExhaustiveModifierList] = useIndexedState(
    useState(
      productEntry.instances.map(
        (pi) =>
          (catalogSelectors.productInstance(pi) as IProductInstance).displayFlags.order
            .suppress_exhaustive_modifier_list,
      ),
    ),
  );
  ...
  const getProductInstanceEditor = useCallback(
    (i: number) => (
      <Accordion
        sx={{ p: 2 }}
        key={i}
        expanded={expandedPanels[i] && copyPIFlags[i]}
        onChange={(_e, ex) => {
          setExpandedPanel(i)(ex);
        }}
      >
              ...
              <FormControlLabel
                ...
                control={
                  <Switch
                    ...
                    onChange={(e) => {
                      setCopyPIFlag(i)(e.target.checked);
                    }}
                    name="Copy"
                  />
                }
    ...
    [
      ...
      piMenuOrdinals,
      piMenuPriceDisplays,
      piMenuShowModifierOptionss,
      piMenuSuppressExhaustiveModifierLists,
      piModifierss,
      piOrderAdornments,
      piOrderMenuHides,
      piOrderOrdinals,
      piOrderPriceDisplays,
      piOrderSuppressExhaustiveModifierLists,
      piOrdinals,
      piShortcodes,
      piOrderSkipCustomizations,
      setCopyPIFlag,
      setExpandedPanel,
      setPiDescription,
      setPiDisplayName,
      ...
      setPiMenuHide,
      setPiMenuOrdinal,
      setPiMenuPriceDisplay,
      setPiMenuShowModifierOptions,
      setPiMenuSuppressExhaustiveModifierList,
      setPiModifiers,
      setPiOrderAdornment,
      setPiOrderMenuHide,
      setPiOrderOrdinal,
      setPiOrderPriceDisplay,
      setPiOrderSuppressExhaustiveModifierList,
      setPiOrdinal,
      setPiShortcode,
      setPiOrderSkipCustomization,
      bakeDifferentialMax,
      bakeMax,
      disabled,
      externalIds,
      ...
      availability,
      timing,
      piPosSkipCustomizations,
      setPiPosSkipCustomization,
    ],
  );
  ...
  const copyProduct = async () => {
    if (!isProcessing) {
      ...
      try {
        ...
        const productCopyBody: CreateProductBatchRequest = {
          product: {
            price,
            serviceDisable,
            externalIDs: externalIds,
          ...
          instances: [
            ...
            ...productEntry.instances.flatMap((_x, i) =>
              copyPIFlags[i] && indexOfBase !== i ? [getUncommittedProductInstanceForIndex(i)] : [],
            ),
          ],
        };
        ...
        if (response.status === 201) {
          enqueueSnackbar(
            `Created product instances: ${productEntry.instances.flatMap((_, i) => (copyPIFlags[i] ? [piDisplayNames[i]] : [])).join(', ')}.`,
          );
