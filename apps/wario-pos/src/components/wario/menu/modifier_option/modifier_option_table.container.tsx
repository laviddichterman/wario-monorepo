      ...
      columns={[
        {
          ...
          getActions: (params: GridRowParams<IOption>) => {
            ...
            const DISABLE_MODIFIER_OPTION_UNTIL_EOD = (<GridActionsCellItem
              ...
              onClick={() => { openModifierOptionDisableUntilEod(params.row.id); }}
              showInMenu
            />)
            const DISABLE_MODIFIER_OPTION = (<GridActionsCellItem
              icon={<Tooltip title={`Disable ${title}`}><Cancel /></Tooltip>}
              label={`Disable ${title}`}
              onClick={() => { openModifierOptionDisable(params.row.id); }}
              showInMenu
            />)
            ...
            return DisableDataCheck(params.row.disabled, [], currentTime).enable !== DISABLE_REASON.ENABLED ? [EDIT_MODIFIER_OPTION, ENABLE_MODIFIER_OPTION, DELETE_MODIFIER_OPTION] : [EDIT_MODIFIER_OPTION, DISABLE_MODIFIER_OPTION_UNTIL_EOD, DISABLE_MODIFIER_OPTION, DELETE_MODIFIER_OPTION];
          }
        },
        ...
        { headerName: "EnableFxn", field: "enable", renderCell: (params) => <ProductInstanceFunctionName {...params} /> },
        // we pass null instead of actually passing the availability because we want to make a decision based on just the .disabled value
        { headerName: "Disabled", field: "disabled", valueGetter: (v: IWInterval | null) => (v !== null && DisableDataCheck(v, [], currentTime).enable !== DISABLE_REASON.ENABLED ? (v.start > v.end ? "True" : `${format(v.start, "MMMM dd, y hh:mm a")} to ${format(v.end, "MMMM dd, y hh:mm a")}`) : "False") },
      ]}
