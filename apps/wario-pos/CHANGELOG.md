...
- a35cd08: Implemented comprehensive features for the Seating Layout Builder:
  - **Undo/Redo**: Full history stack with keyboard shortcuts.
  - **Delete**: Cascade delete for sections and floors with confirmation.
  - **Grid Constraints**: Prevent elements from leaving the canvas.
  - **Save Fix**: Robust upsert logic using discriminated unions.

- 21ae8ee: Add navigation guard and improve naming UX for Seating Layout Builder
...
- 0dc465f: - **Architecture**: Unified Order Drawer state management using `orderDrawerAtom` (Jotai), replacing local state and prop drilling in `OrderManager` and `OrderCalendar`.
...
- 17dc292: Refactor CRUD containers and config components (KV Store, Lead Times, Block Off, Settings) to use TanStack Query mutations instead of manual `fetch` calls. Created `useConfigMutations` and `useConfigQueries` hooks.
