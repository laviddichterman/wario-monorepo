...
import {
  useProductHasSelectableModifiersByProductInstanceId,
  useProductMetadataFromProductInstanceIdWithCurrentFulfillmentData,
...
function ShopClickableProductDisplay({
  ...
  setScrollToOnReturn,
  ...props
}: ShopClickableProductDisplayProps & BoxProps) {
  ...
  const onProductSelection = useCallback(() => {
    ...
    if (productInstance && productMetadata) {
      const productCopy: WProduct = {
        p: CreateWCPProduct(productInstance.productId, productInstance.modifiers),
        m: structuredClone(productMetadata),
      };
      if (
...
function AccordionSubCategory({
  ...
  const productInstancesIdsInCategory = useProductInstancesInCategory(
    categoryId,
    'Order',
    serviceDateTime,
    selectedService,
...
export function WShopForProductsStage({ categoryId, setScrollToOnReturn }: WShopForProductsStageProps) {
  const selectedService = useFulfillmentStore(selectSelectedService) as string;
  const serviceDateTime = useFulfillmentStore(selectServiceDateTime) as Date;
  const populatedSubcategories = usePopulatedSubcategoryIdsInCategory(
    categoryId,
    'Order',
    serviceDateTime,
    selectedService,
