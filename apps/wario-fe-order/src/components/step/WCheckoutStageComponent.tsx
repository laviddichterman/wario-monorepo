...
const TIP_SUGGESTION_15: TipSelection = { value: 0.15, isSuggestion: true, isPercentage: true } as const;
const TIP_SUGGESTION_20: TipSelection = { value: 0.2, isSuggestion: true, isPercentage: true } as const;
const TIP_SUGGESTION_25: TipSelection = { value: 0.25, isSuggestion: true, isPercentage: true } as const;
...
export default function WCheckoutStage() {
  ...
  const cardTokenizeResponseReceived = useCallback(
    (props: Square.TokenResult, _verifiedBuyer?: Square.VerifyBuyerResponseDetails | null) => {
      if (props.status === 'OK') {
  ...
  const tipSuggestionsArray = useMemo(
    () =>
      TIP_SUGGESTIONS.slice(
        autogratEnabled ? 1 : 0,
        autogratEnabled ? TIP_SUGGESTIONS.length : TIP_SUGGESTIONS.length - 1,
      ),
    [autogratEnabled],
  );
  const isCustomTipSelected = useMemo(() => selectedTip?.isSuggestion === false, [selectedTip]);
  const [customTipAmount, setCustomTipAmount] = useState<string>(
    MoneyToDisplayString(ComputeTipValue(selectedTip || TIP_SUGGESTION_20, tipBasis), false),
  );
        ...
        return (
          <>
            <Typography variant="body2">
              Payment of {MoneyToDisplayString(payment.amount, true)} received{' '}
              {payment.payment.last4 ? ` from card ending in: ${payment.payment.last4}!` : '!'}
            </Typography>
          </>
        );
      case PaymentMethod.StoreCredit:
        const paymentBalance = {
          amount: payment.payment.balance.amount - payment.amount.amount,
          currency: payment.amount.currency,
        };
        return (
          <>
            <Typography variant="h6">
              Digital Gift Card number <Typography sx={{ textTransform: 'none' }}>{payment.payment.code}</Typography>{' '}
              debited {MoneyToDisplayString(payment.amount, true)}.
            </Typography>
            <Typography variant="body2">
              {paymentBalance.amount === 0
                ? 'No balance remains.'
                : `Balance of ${MoneyToDisplayString(paymentBalance, true)} remains.`}
            </Typography>
          </>
        );
    }
  ...
  const setCustomTipHandler = (value: string) => {
    ...
    if (
      ...
      isNaN(numericValue) ||
      numericValue < 0 ||
      (autogratEnabled && Math.round(numericValue * 100) < TwentyPercentTipValue.amount)
    ) {
      incrementTipFixes();
      resetCustomTip();
    } else {
      ...
      {!submitOrderMutation.isSuccess ? (
        <Box>
              ...
              <Grid container sx={{ py: 2 }}>
                {tipSuggestionsArray.map((tip: TipSelection, i: number) => (
                  <Grid key={i} sx={{ px: 0.5 }} size={4}>
                    <WarioToggleButton
                      ...
                      fullWidth
                      onClick={() => {
                        onSelectSuggestedTip(tip);
                      }}
                    >
                      <Grid container sx={{ py: 2 }}>
                        <Grid size={12}>
                          <Typography sx={{ color: 'white' }} variant="h4">
                            {(tip.value as number) * 100}%
                          </Typography>
                        </Grid>
                        <Grid size={12}>
                          <Typography sx={{ color: 'white' }} variant="subtitle2">
                            {MoneyToDisplayString(ComputeTipValue(tip, tipBasis), false)}
                          </Typography>
                        </Grid>
                      </Grid>
                    </WarioToggleButton>
                ...
                <Grid sx={{ px: 0.5, pt: 1 }} size={12}>
                  <WarioToggleButton
                    ...
                    onClick={() => {
                      setCustomTipHandler(customTipAmount);
                    }}
                  >
                    <Grid container>
                      <Grid size={12}>
                        ...
                        {isCustomTipSelected ? (
                          <Input
                            ...
                            onBlur={(e: React.FocusEvent<HTMLInputElement>) => {
                              setCustomTipHandler(e.target.value);
                            }}
                            type="number"
                            inputProps={{
                              inputMode: 'decimal',
                              min: 0,
                              sx: { pt: 0, textAlign: 'center', color: 'white' },
                              step: 1,
                            }}
                          />
                        ) : (
          ...
          <Box>
            ...
            <Grid container>
              <Grid container sx={{ px: 2, pb: 4 }} size={12}>
                <StoreCreditSection />
              </Grid>
              <Grid size={12}>
                {balance.amount > 0 ? (
                ...
                <GooglePay>Pay with Google Pay</GooglePay> */}
                  ...
                  <WarioButton disabled={submitOrderMutation.isPending} fullWidth onClick={submitNoBalanceDue}>
                    Submit Order
                  </WarioButton>
                )}
                {squareTokenErrors.length > 0 &&
                  squareTokenErrors.map((e, i) => (
                    <Grid key={i} size={12}>
                      <ErrorResponseOutput key={`${i.toString()}TokenErrors`}>{e.message}</ErrorResponseOutput>
                    </Grid>
                  ))}
                {submitOrderMutation.isError &&
                  submitOrderMutation.error.errors.map((e, i) => (
                    <Grid key={i} size={12}>
                      <ErrorResponseOutput key={`${i.toString()}Payment`}>{e}</ErrorResponseOutput>
                    </Grid>
                  ))}
                <div>
                  Note: Once orders are submitted, they are non-refundable. We will attempt to make any changes
                  requested, but please do your due diligence to check the order for correctness!
                </div>
              </Grid>
            </Grid>
            <Navigation
